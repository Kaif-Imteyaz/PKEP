...
export function Layout({
  children,
  onViewChange,
}: { children: React.ReactNode; onViewChange: (view: string) => void }) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (data.user) {
        ...
        setUserName(name || "Amitabh Bachchan") // Default name if not available
      }
    }
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Check if the click is outside the notifications menu
  ...
  const navItems = [
    ...
    { id: "badges", label: "My Badges", icon: Award, description: "Achievement badges" },
  ]

  // Unread notifications indicator - in a real app, fetch this from backend
    ...
    <div className="min-h-screen bg-neutral-50 flex flex-col">
              ...
              {notificationsOpen && (
                <div className="origin-top-right absolute right-0 mt-2 w-80 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                  ...
                  <div className="max-h-60 overflow-y-auto">
                    <div className="py-2 px-3 bg-blue-50 border-l-4 border-blue-500">
                      <div className="flex justify-between">
                        <p className="text-sm font-medium text-blue-800">New Badge Earned! 🏅</p>
                        <span className="text-xs text-blue-600">2h ago</span>
                      </div>
                      <p className="text-xs text-blue-700 mt-1">
                        Congratulations! You've earned the 'Efficiency Expert' badge.
                      </p>
                    </div>
                    <div className="py-2 px-3 hover:bg-gray-50">
                      <div className="flex justify-between">
                        <p className="text-sm font-medium text-gray-800">Weekly Progress Update</p>
                        <span className="text-xs text-gray-500">1d ago</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">
                        Your application processing time improved by 15% this week.
                      </p>
                    </div>
                    <div className="py-2 px-3 hover:bg-gray-50">
                      <div className="flex justify-between">
                        <p className="text-sm font-medium text-gray-800">Upcoming Meeting</p>
                        <span className="text-xs text-gray-500">1d ago</span>
                      </div>
                      <p className="text-xs text-gray-600 mt-1">
                        Meeting with Dharmendra scheduled for tomorrow at 2:00 PM.
                      </p>
                    </div>
                  </div>
                  <div className="py-1 border-t border-gray-200">
                    <button
                      onClick={() => handleViewChange("notifications")}
                      className="block w-full text-center px-4 py-2 text-sm text-primary-600 hover:bg-gray-50"
                    >
                ...
                <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                  <div className="py-1">
                    <button
                      ...
                      className="block w-full text-left px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-100 transition-colors duration-150"
                    >
                      Your Profile
                    </button>
                    <button
      ...
      <div className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 z-50">
        <div className="flex flex-col flex-grow bg-primary-700 pt-5 pb-4 overflow-y-auto">
          ...
          <div className="text-xs text-primary-50 px-4 mt-1">Peer Knowledge Exchange Platform</div>
                ...
                >
                  <item.icon
                    className={`mr-3 h-5 w-5 transition-colors ${
                      activeView === item.id ? "text-white" : "text-primary-50 group-hover:text-white"
                    }`}
                  />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">
                A collaborative initiative to enhance public service delivery through peer learning and knowledge
                exchange.
              </p>
            </div>
            <div>
              ...
              <div className="mt-2 text-sm text-gray-600">
                <p>Department of Good Governance and Information Technology</p>
